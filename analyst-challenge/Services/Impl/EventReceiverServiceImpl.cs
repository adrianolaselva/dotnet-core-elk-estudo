...
namespace analyst_challenge.Services.Impl
{
    public class EventReceiverServiceImpl : IEventReceiverService
    {
        ...
        public EventReceiverServiceImpl(ILogger<EventReceiverServiceImpl> logger, IAmazonSQS sqs, IConfiguration configuration)
        {
            _logger = logger;
            _sqs = sqs;
        ...
        public EventReceiver Create(EventReceiver eventReceiver)
        {
            try
            {
                ...
                _logger.Log(LogLevel.Information, $"Evento regitrado com sucesso => [{JsonConvert.SerializeObject(eventReceiver)}]");
            }
            catch (Exception exception)
            {
                _logger.Log(LogLevel.Error, exception.StackTrace);
                throw new Exception($"Falha ao registrar evento => [{JsonConvert.SerializeObject(eventReceiver)}, {exception.Message}]");
            }
